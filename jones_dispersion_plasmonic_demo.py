#!/usr/bin/env python3
"""
2D Jones Dispersion Demo - Plasmonic Features with Gold

This script demonstrates plasmonic dispersion features using a Gold film
to show surface plasmons and other dispersive effects in the 2D E-q plot.

Author: Ding Xu (Physical Chemistry Researcher)
Based on original Jones_disperion.py with plasmonic enhancements
"""
...
def calc_dispersion_fast(layer_info, fixed_axis, fixed_val,
                        E_start, E_stop, q_start, q_stop,
                        a, b, delta_phi_deg, resolution='medium'):
    """
    Calculate 2D dispersion relations with adjustable resolution
...
def plot_dispersion_enhanced(E_vals, q_vals, Rs, Rp, S0, S1, S2, S3, phi, chi, 
                           fixed_axis, fixed_val, layer_info, save_figure=True):
    """Create enhanced 2D dispersion plots with better styling"""
    fig, axes = plt.subplots(2, 4, figsize=(20, 10))
    
    # Plot parameters
    titles = ['Rs (s-polarized)', 'Rp (p-polarized)', 'S0 (Intensity)', 'S1 (Linear H-V)', 
    ...
    for idx, (ax, title, d) in enumerate(zip(axes.flatten(), titles, data)):
        im = ax.pcolormesh(q_vals, E_vals, d, shading='auto', cmap=cmaps[idx], 
                          vmin=vmin_vals[idx], vmax=vmax_vals[idx])
        ax.set_title(title, fontsize=12, fontweight='bold')
        ax.set_xlabel('q (μm⁻¹)', fontsize=11)
    ...
    fig.suptitle(f'2D Jones Dispersion: {layer_str}\n' + 
                 f'Fixed {fixed_axis} = {fixed_val} μm⁻¹', 
    ...
    if save_figure:
        ...
        filename = f'jones_dispersion_2d_{materials}_{fixed_axis}_{fixed_val:.1f}.png'
...
def main():
    ...
    print("=== 2D Jones Dispersion Analysis - Plasmonic Demo ===")
    print("Showing surface plasmon dispersion in Gold film\n")
    ...
    E_start, E_stop = 0.5, 4.0     # Energy range (eV) - covers plasmonic region
    q_start, q_stop = 0.0, 30.0    # q-vector range (μm⁻¹) - higher for plasmons
    ...
    print(f"  Energy range: {E_start} - {E_stop} eV (plasmonic region)")
    ...
    results = calc_dispersion_fast(layer_info, fixed_axis, fixed_val,
                                  E_start, E_stop, q_start, q_stop,
                                  a, b, delta_phi_deg, resolution='medium')
    ...
    # Plot results
    print("\nCreating enhanced dispersion plots...")
    plot_dispersion_enhanced(E_vals, q_vals, Rs, Rp, S0, S1, S2, S3, phi, chi,
                           fixed_axis, fixed_val, layer_info, save_figure=True)
    
    print("\n=== Plasmonic Analysis Complete ===")
    print(f"Generated 2D dispersion plots with {len(E_vals)} × {len(q_vals)} data points")
    print("This shows surface plasmon dispersion features in the Gold film")
    print("Look for:")
    print("  - Surface plasmon resonances (high absorption bands)")
    print("  - Dispersion curves in the Rp plot")
    print("  - Enhanced field effects in the Stokes parameters")
